	...
	internal partial class NewQuestionDlg : Form
	{
		...
		private void m_scrPsgReference_PassageChanged(BCVRef newReference)
		{
			...
			if (m_existingStartRef != newRef)
			{
				...
				// If the start verse is sey beyond the end verse, we get into a funny state where the
				...
				// (which will be used in SetCurrentSelections) doesn't know that. Unfortunately, it's we have
				// a chicken-and-egg problem because we need to have the current sections to repopulate it, so
