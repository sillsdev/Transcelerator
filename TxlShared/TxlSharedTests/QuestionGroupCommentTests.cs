...
namespace TxlSharedTests
{
	...
	/// <see cref="QuestionGroupHelperTests"/>.
	/// </summary>
	[TestFixture]
	public class QuestionGroupCommentTests
	{
		[Test]
		public void Constructor_BasedOnEitherOrMatchOnly_ReturnsObjectRepresentingMatchInfo()
		{
			var matches = QuestionGroupComment.RegexRedundantGroupNote.Matches(
			...
			var comment = new QuestionGroupComment(matches[0]);
		...
		public void Constructor_BasedOnGroupMatchOnly_ReturnsObjectRepresentingMatchInfo()
		{
			...
			Assert.IsFalse(comment.IsForGroup);
			Assert.That(comment.NumberOfQuestionsInBothGroups, Is.EqualTo(3));
			Assert.That(comment.QuestionWithCommentAboutRedundancy, Is.Null);
			Assert.That(comment.Index, Is.EqualTo(-1));
			Assert.That(comment.FirstGroupLetter, Is.EqualTo('C'));
			Assert.That(comment.SecondGroupLetter, Is.EqualTo('D'));
